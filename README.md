...
- Passwords are securely hashed (bcrypt) — never stored as plain text
- Auto-login — once logged in, the browser remembers you (saved token) until you log out
...
## How login works
- Accounts are stored in a `users.json` file on the server (created automatically the first time someone signs up)
- Passwords are hashed with **bcrypt** before saving — even you (the server owner) can't see anyone's real password
- On login, the server gives the browser a random **session token**, which gets saved in `localStorage`. Next time the person opens the site, that token auto-logs them in — no need to retype password every time — until they hit "Log Out"
- ⚠️ **Note:** `users.json` lives on Render's filesystem, which is *not* permanent — if you redeploy or the free-tier service resets, accounts may be wiped. Fine for personal/hobby use; for a "real" production app you'd swap this for a proper database (e.g. PostgreSQL) later.
